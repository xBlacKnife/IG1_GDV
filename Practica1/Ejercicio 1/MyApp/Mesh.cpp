	...
	GLdouble ladoIni, GLdouble incrLado, GLuint numVert)
{
		...
		for (int i = 1; i < numVert; i++)
		{
			...
			m->colors[i] = dvec4(255.0, 0.0, 80.0, 0.0);
		}
		return m;
}
...
Mesh * Mesh::generateRectangleText(GLdouble l, int repX, int repY)
{
	
	Mesh *m = generateCuadrado(l);
	m->texCoords = new dvec2[m->numVertices];
	m->texCoords[0] = dvec2(0, repY + 1);
	m->texCoords[1] = dvec2(0, 0);
	m->texCoords[2] = dvec2(repX + 1, repY + 1);
	m->texCoords[3] = dvec2(repX + 1, 0);
	return m;
}

Mesh * Mesh::generateContCuboText(GLdouble l)
{
...
Mesh * Mesh::generateTriPyramidText(GLdouble r, GLdouble h)
{
	...
	m->texCoords = new dvec2[m->numVertices];
	m->texCoords[0] = dvec2(0.5, 0);
	m->texCoords[1] = dvec2(0, 1);
	m->texCoords[2] = dvec2(1/3, 1);
	m->texCoords[3] = dvec2(2/3, 1);
